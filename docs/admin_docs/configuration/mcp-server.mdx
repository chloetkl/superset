...
- The resolved MCP tool name (`mcp_tool`) when tool search proxies a `call_tool` request to the underlying tool
- The error type (`error_type`) when a tool call raises an exception
...
|------------|---------|---------|
| `StructuredContentStripperMiddleware` | Strips `structuredContent` from responses for Claude.ai bridge compatibility. Gracefully returns an empty tool list when inner middleware raises during `tools/list` | Enabled |
| `LoggingMiddleware` | Logs each tool call with user, parameters, duration, resolved `mcp_tool` name (for `call_tool` proxies), and `error_type` on failures | Enabled |
