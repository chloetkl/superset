...
| `get_chart_preview` | Generate a chart preview (URL or Vega-Lite spec). Preview content is returned via the structured `content` field |
...
| `update_chart_preview` | Update or create a chart preview without saving. `form_data_key` is optional — omit it to create a fresh preview from `config` + `dataset_id` |
...
| `add_chart_to_existing_dashboard` | Add a chart to an existing dashboard. Use `target_tab` to place it in a specific tab (by name or component ID); returns an error listing available tabs if the target is not found |
...
| `open_sql_lab_with_context` | Open SQL Lab with a pre-populated query. Accepts an optional `title` (max 256 chars) to label the tab |
